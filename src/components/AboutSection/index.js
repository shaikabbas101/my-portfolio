import './index.css'
import {Component} from 'react'

const skillsData = [
  {
    id: 0,
    name: 'HTML',
  },
  {
    id: 1,
    name: 'Java Script',
  },
  {
    id: 2,
    name: 'CSS',
  },
  {
    id: 3,
    name: 'Python',
  },
  {
    id: 4,
    name: 'React',
  },
  {
    id: 5,
    name: 'NodeJS',
  },
  {
    id: 6,
    name: 'Bootstrap',
  },

  {
    id: 7,
    name: 'SQL',
  },
  {
    id:8,
    name:'Express'
  }
]

const educationData = [
  {
    id: 0,
    qualification: 'Graduation',
    qualificationSource: 'B.Tech(Mechanical Engineering)',
  },
  {
    id: 1,
    qualification: 'Intermediate',
    qualificationSource: `Rao's  Jr College`,
  },
  {
    id: 2,
    qualification: 'High School',
    qualificationSource: 'Keshava Reddy EM School',
  },
]

class AboutSection extends Component {
  renderSkillsList = () => (
    <ul className="skills-list">
      {skillsData.map(skill => (
        <li className="skill-item" key={skill.id}>
          <p className="skill">{skill.name}</p>
        </li>
      ))}
    </ul>
  )

  renderEducationDetailsList = () =>
    educationData.map(education => (
      <li key={education.id}>
        <div className="education-container">
          <p className="qualification">{education.qualification}</p>
          <p className="education-place">{education.qualificationSource}</p>
        </div>
      </li>
    ))

  render() {
    return (
      <div>
        <h1 className="about-section-heading">About</h1>
        <p className="about-section-description">
          I’m 24 years old, Graduated in Mechanical Engineering from RGMCET
          ,Neravada. later joined web development course which became my center of interest.
        </p>
        <div className="education-and-skills-container">
          <div className="education-details-container">
            <h1 className="education-and-skill-heading">Education</h1>
            <ul className="education-details-list">
              {this.renderEducationDetailsList()}
            </ul>
          </div>
          <div className="skills-container">
            <h1 className="education-and-skill-heading">Skills</h1>
            {this.renderSkillsList()}
          </div>
        </div>
      </div>
    )
  }
}

export default AboutSection
